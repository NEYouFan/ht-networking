...
//  Main Modification: Add a method isInvalidType to check whether object type is valid before calling method valueForKeyPath:keyPath.
...
- (id)valueForKey:(NSString *)key
{
    ...
    if (firstChar == 's' && [key isEqualToString:RKSelfKey]) {
        ...
        return [self isInvalidType:_object] ? nil : [_object valueForKey:key];
...
- (id)valueForKeyPath:(NSString *)keyPath
{
    ...
    if (firstChar == 's' && [keyPath hasPrefix:RKSelfKeyPathPrefix]) {
        ...
        return [self isInvalidType:_object] ? nil : [_object valueForKeyPath:keyPath];
...
// LWANG COMMENT: If object is a NSSting or NSNumber Object, it would crash while calling [object valueForKeyPath:keyPath].
// But there is no way to verify a key-path prior to invoking valueForKeyPath:.
- (BOOL)isInvalidType:(id)object {
...
- (BOOL)applyAttributeMapping:(RKAttributeMapping *)attributeMapping withValue:(id)value
{
    ...
    // LWANG Comment: 这里可以将value作一些必要的类型转换. 例如, 将NSNumber转NSString或者NSString转NSNumber.
...
- (void)main
{
    // LWANG Comments: 在该方法中执行具体的Mapping过程.
