...
- (void)performMappingOnResponseWithCompletionBlock:(void(^)(RKMappingResult *mappingResult, id responseObject, NSError *error))completionBlock
{
    ...
    if ([fetchRequests count] && [self canSkipMapping]) {
        ...
        // TODO: 这里会取不到response Object. 在CoreData存储情况下暂时不支持获取responseObject.
        completionBlock(mappingResult, nil, nil);
        return;
    }
    ...
    [self.responseMapperOperation setDidFinishMappingBlock:^(RKMappingResult *mappingResult, id responseObject, NSError *responseMappingError) {
        if ([weakSelf isCancelled]) return completionBlock(mappingResult, responseObject, responseMappingError);
        ...
        if (weakSelf.targetObjectID
            ...
            if (! success || [weakSelf isCancelled]) {
                return completionBlock(nil, responseObject, error);
            }
        }
        ...
        if (mappingResult) {
            ...
            return completionBlock((RKMappingResult *)refetchingMappingResult, responseObject, nil);
        }
        completionBlock(nil, responseObject, responseMappingError);
    }];
