...
/**
 ...
 Each object manager is configured with a base URL that defines the URL that all request sent through the manager will be relative to. The base URL is configured directly through the `managerWithBaseURL:` method or is inherited from a `RKRequestProvider` object if the manager is initialized via the `initWithRequestProvider:` method. The base URL can point directly at the root of a URL or may include a path.
 ...
 Routing interfaces are provided by the `RKRouter` class. Each object manager is in initialized with an `RKRouter` object with a baseURL equal to the baseURL of the underlying `RKRequestProvider` object. Each `RKRouter` instance maintains an `RKRouteSet` object that manages a collection of `RKRoute` objects. Routes are defined in terms of a path pattern.
 ...
 * `enqueueObjectRequestOperation:` - Invoked to enqueue all operations constructed by the manager that are to be started as soon as possible. Provide a subclass implementation if you wish to work with object request operations as they are be enqueued.
 ...
 You can also customize the HTTP details by subclassing `RKRequestProvider` and using an instance of your subclassed client to initialize the manager.
 
 @warning Note that when subclassing `RKRequestProvider` to change object manager behaviors it is not possible to alter the paramters of requests that are constructed on behalf of the manager. This is because the object manager handles its own serialization and construction of the request body, but defers to the `RKRequestProvider` for all other details (such as default HTTP headers, etc).
...
/**
 Creates and returns a new `RKObjectManager` object initialized with a new `RKRequestProvider` object that was in turn initialized with the given base URL. The RestKit defaults are applied to the object manager.
 
 When initialized with a base URL, the returned object manager will have a `requestSerializationMIMEType` with the value of `RKMIMETypeFormURLEncoded` and the underlying `RKRequestProvider` will have a default value for the 'Accept' header set to `RKMIMETypeJSON`, and the `AFJSONRequestOperation` class will be registered.
 
 @param baseURL The base URL with which to initialize the `RKRequestProvider` object
 @return A new `RKObjectManager` initialized with an `RKRequestProvider` that was initialized with the given baseURL.
 */
+ (instancetype)managerWithBaseURL:(NSURL *)baseURL;

/**
 Initializes the receiver with the RKRequestProvider, adopting the network configuration from the request provider.
 
 This is the designated initializer. If the `sharedManager` instance is `nil`, the receiver will be set as the `sharedManager`. The default headers and parameter encoding of the given RKRequestProvider are adopted by the receiver to initialize the values of the `defaultHeaders` and `requestSerializationMIMEType` properties.

 @param requestProvider The RKRequestProvider with which to initialize the receiver.
 @return The receiver, initialized with the given client.
 */
- (instancetype)initWithRequestProvider:(RKRequestProvider *)requestProvider NS_DESIGNATED_INITIALIZER;
...
/**
 The RKRequestProvider with which the receiver makes requests. Keep old property name to avoid modifing RestKit code too much.
 */
@property (nonatomic, strong, readwrite) RKRequestProvider *requestProvider;

/**
 The config delegate is used to config requests created by the manager.
 
 */
@property (nonatomic, weak) id<RKRequestProviderConfigDelegate> configDelegate;

/**
 *  检查结果是否有效. 如果validResultBlock不为nil, 则对结果进行有效性检查; 如果结果无效，那么转到Error分支.
 *  默认情况下为nil，即如果执行过程正确但是未取到期待的结果，仍然是通过success block回调.
 *  不影响已创建好，仅仅用Manager进行调度的operation; 影响所有通过Manager创建的operation.
 */
@property (nonatomic, copy) HTValidResultBlock validResultBlock;

/**
 The base URL of the underlying RequestProvider.
 */
...
/**
 ...
 The returned dictionary contains all of the default headers set on the underlying `RKRequestProvider` object and the value of the 'Accept' header set on the object manager, if any.
 ...
 **Default**: `RKMIMETypeFormURLEncoded` or the value of the parameter encoding for the underlying `RKRequestProvider`.
 */
@property (nonatomic, strong) NSString *requestSerializationMIMEType;

/**
 Sets a default header on the RequestProvider for the HTTP "Accept" header to specify the preferred serialization format for retrieved data.
 
 This method is a convenience method whose implementation is equivalent to the following example code:
 
    [manager.RequestProvider setDefaultHeader:@"Accept" value:MIMEType];
...
/**
 ...
 If the given path is nil, the router is searched for a class route with the class of the object andthe method. The path pattern of the retrieved route is interpolated with the object and the resulting path is appended to the RequestProvider's base URL and used as the request URL.
 ...
 @param path The path to be appended to the RequestProvider's base URL and used as the request URL. If nil, the router is consulted.
...
/**
 ...
 This method wraps the underlying `RKRequestProvider` method `multipartFormRequestWithMethod:path:parameters:constructingBodyWithBlock` and adds routing and object parameterization.
 ...
 @see [RKRequestProvider multipartFormRequestWithMethod:path:parameters:constructingBodyWithBlock]
 */
...
/**
 ...
 @param routeName The name of the route object containing the path pattern which is to be interpolated against the given object, appended to the RequestProvider's base URL and used as the request URL.
...
/**
 ...
 @param path The path to be appended to the RequestProvider's baseURL and set as the URL of the request. If nil, the router is consulted.
 @param parameters The parameters to be either set as a query string for `GET` requests, or reverse merged with the parameterization of the object and set as the request HTTP body.
...
/**
 Cancels all operations in the object manager's operation queue whose requests match the specified HTTP method.
 
 @param method The HTTP method to match for the cancelled requests, such as `RKRequestMethodGET`, `RKRequestMethodPOST`, `RKRequestMethodPUT`, `RKRequestMethodPatch`, or `RKRequestMethodDELETE`. If `RKRequestMethodAny`, all object request operations with URLs matching the given path pattern will be cancelled.
 
 */
- (void)cancelAllObjectRequestOperationsWithMethod:(RKRequestMethod)method;
...
/**
 ...
 @param object The object to be interpolated against the path pattern of the `RKRoute` object retrieved with the given name. Used to compute the path to be appended to the RequestProvider's base URL and used as the request URL. May be nil.
 @param parameters The parameters to be encoded and appended as the query string for the request URL. May be nil.
 @param success A block object to be executed when the object request operation finishes successfully. This block has no return value and takes two arguments: the created object request operation and the mapped result created from object mapping the response data of request.
 @param failure A block object to be executed when the request operation finishes unsuccessfully, or that finishes successfully, but encountered an error while parsing the resonse data. This block has no return value and takes two arguments:, the created request operation and the `NSError` object describing the network or parsing error that occurred.
...
/**
 ...
 @param path The path to be appended to the RequestProvider's base URL and used as the request URL. If nil, the request URL will be obtained by consulting the router for a route registered for the given object's class and the `RKRequestMethodGET` request method.
 @param parameters The parameters to be encoded and appended as the query string for the request URL.
 @param success A block object to be executed when the object request operation finishes successfully. This block has no return value and takes two arguments: the created object request operation and the `RKMappingResult` object created by object mapping the response data of request.
 @param failure A block object to be executed when the request operation finishes unsuccessfully, or that finishes successfully, but encountered an error while parsing the resonse data. This block has no return value and takes two arguments:, the created request operation and the `NSError` object describing the network or parsing error that occurred.
...
/**
 ...
 @param object The object with which to construct the object request operation. If `nil`, then the path must be provided.
 @param path The path to be appended to the RequestProvider's base URL and used as the request URL. If nil, the request URL will be obtained by consulting the router for a route registered for the given object's class and the `RKRequestMethodPOST` method.
...
/**
 ...
 @param object The object with which to construct the object request operation. If `nil`, then the path must be provided.
 @param path The path to be appended to the RequestProvider's base URL and used as the request URL. If nil, the request URL will be obtained by consulting the router for a route registered for the given object's class and the `RKRequestMethodPUT` method.
 @param parameters The parameters to be reverse merged with the parameterization of the given object and set as the request body.
 @param success A block object to be executed when the object request operation finishes successfully. This block has no return value and takes two arguments: the created object request operation and the `RKMappingResult` object created by object mapping the response data of request.
 @param failure A block object to be executed when the request operation finishes unsuccessfully, or that finishes successfully, but encountered an error while parsing the resonse data. This block has no return value and takes two arguments:, the created request operation and the `NSError` object describing the network or parsing error that occurred.
...
/**
 ...
 @param object The object with which to construct the object request operation. If `nil`, then the path must be provided.
 @param path The path to be appended to the RequestProvider's base URL and used as the request URL. If nil, the request URL will be obtained by consulting the router for a route registered for the given object's class and the `RKRequestMethodPATCH` method.
 @param parameters The parameters to be reverse merged with the parameterization of the given object and set as the request body.
 @param success A block object to be executed when the object request operation finishes successfully. This block has no return value and takes two arguments: the created object request operation and the `RKMappingResult` object created by object mapping the response data of request.
 @param failure A block object to be executed when the request operation finishes unsuccessfully, or that finishes successfully, but encountered an error while parsing the resonse data. This block has no return value and takes two arguments:, the created request operation and the `NSError` object describing the network or parsing error that occurred.
...
/**
 ...
 @param object The object with which to construct the object request operation. If `nil`, then the path must be provided.
 @param path The path to be appended to the RequestProvider's base URL and used as the request URL. If nil, the request URL will be obtained by consulting the router for a route registered for the given object's class and the `RKRequestMethodDELETE` request method.
 @param parameters The parameters to be encoded and appended as the query string for the request URL.
 @param success A block object to be executed when the object request operation finishes successfully. This block has no return value and takes two arguments: the created object request operation and the `RKMappingResult` object created by object mapping the response data of request.
 @param failure A block object to be executed when the request operation finishes unsuccessfully, or that finishes successfully, but encountered an error while parsing the resonse data. This block has no return value and takes two arguments:, the created request operation and the `NSError` object describing the network or parsing error that occurred.
