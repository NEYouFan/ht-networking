...
static NSString *RKMIMETypeFromRKRequestProviderParameterEncoding(RKRequestProviderParameterEncoding encoding)
{
    switch (encoding) {
        ...
        default:
            RKLogWarning(@"RestKit is unable to infer the appropriate request serialization MIME Type from an `RKRequestProviderParameterEncoding` value of %d: defaulting to `RKMIMETypeFormURLEncoded`", encoding);
            break;
    }
...
{
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:[NSString stringWithFormat:@"-init is not a valid initializer for the class %@, use designated initilizer -initWithRequestProvider:", NSStringFromClass([self class])]
                                 userInfo:nil];
    return [self init];
}

- (instancetype)initWithRequestProvider:(RKRequestProvider *)requestProvider
{
    self = [super init];
    if (self) {
        self.requestProvider = requestProvider;
        self.router = [[RKRouter alloc] initWithBaseURL:requestProvider.baseURL];
        ...
        self.requestSerializationMIMEType = RKMIMETypeFromRKRequestProviderParameterEncoding(requestProvider.parameterEncoding);
...
{
    RKObjectManager *manager = [[self alloc] initWithRequestProvider:[RKRequestProvider requestProviderWithBaseURL:baseURL]];
...
{
    [self.requestProvider setDefaultHeader:@"Accept" value:MIMEType];
}

- (NSURL *)baseURL
{
    return self.requestProvider.baseURL;
}

- (NSDictionary *)defaultHeaders
{
    return self.requestProvider.defaultHeaders;
}

#pragma mark - Building Requests

/**
 This method is the `RKObjectManager` analog for the method of the same name on `RKRequestProvider`.
 */
                                ...
                                parameters:(NSDictionary *)parameters
{
    ...
    NSDictionary *customizedParameters = [self cutomizedParameters:parameters];
    
    NSMutableURLRequest* request;
    if (customizedParameters && !([method isEqualToString:@"GET"] || [method isEqualToString:@"HEAD"] || [method isEqualToString:@"DELETE"])) {
        // NOTE: If the RequestProvider has been subclasses, then the developer may be trying to perform signing on the request
        NSDictionary *parametersForClient = [self.requestProvider isMemberOfClass:[RKRequestProvider class]] ? nil : customizedParameters;
        request = [self.requestProvider requestWithMethod:method path:path parameters:parametersForClient];
		
        NSError *error = nil;
        NSString *charset = (__bridge NSString *)CFStringConvertEncodingToIANACharSetName(CFStringConvertNSStringEncodingToEncoding(self.requestProvider.stringEncoding));
        ...
        NSData *requestBody = [RKMIMETypeSerialization dataFromObject:customizedParameters MIMEType:self.requestSerializationMIMEType error:&error];
        [request setHTTPBody:requestBody];
	} else {
        request = [self.requestProvider requestWithMethod:method path:path parameters:customizedParameters];
    }
    
	return [self customizedRequest:request];
}

// Parameters一定是叠加
- (NSDictionary *)cutomizedParameters:(NSDictionary *)parameters {
    if (0 == [_requestProvider.defaultParams count]) {
        return parameters;
    }
    
    NSMutableDictionary *customizedParams = [NSMutableDictionary dictionaryWithDictionary:_requestProvider.defaultParams];
    [customizedParams addEntriesFromDictionary:parameters];
    
    return customizedParams;
}

// TODO: 更换RequestProvider有问题. 即使是原来的方式也存在问题. (因为一些通过RequestProvider设置的东西，在更换Client后就失效了)
// 例如- (void)testChangingRequestProvider中, 换掉RequestProvider后AcceptType就不一定默认是Json了.
- (NSMutableURLRequest *)customizedRequest:(NSMutableURLRequest *)request {
    NSMutableURLRequest *customizedRequest = request;
    if ([_configDelegate respondsToSelector:@selector(customRequest:)]) {
        NSURLRequest *customRequest = [_configDelegate customRequest:customizedRequest];
        customizedRequest = [customRequest mutableCopy];
    }
                              ...
                              constructingBodyWithBlock:(void (^)(id <AFMultipartFormData> formData))block
{
    NSString *requestPath = (path) ? path : [[self.router URLForObject:object method:method] relativeString];
    id requestParameters = [self mergedParametersWithObject:object method:method parameters:parameters];
    NSMutableURLRequest *multipartRequest = [self.requestProvider multipartFormRequestWithMethod:RKStringFromRequestMethod(method)
                                                                                       path:requestPath
...
- (void)copyStateToHTTPRequestOperation:(RKHTTPRequestOperation *)operation
{
    // LWANG: set credential and securityPolicy here.
    operation.credential = self.requestProvider.defaultCredential;
    operation.securityPolicy = self.requestProvider.securityPolicy;
}
                                                        ...
                                                        failure:(void (^)(RKObjectRequestOperation *operation, NSError *error))failure
{
    ...
    [self copyStateToHTTPRequestOperation:HTTPRequestOperation];
    ...
    operation.validResultBlock = self.validResultBlock;
    return operation;
}
...
- (void)cancelAllObjectRequestOperationsWithMethod:(RKRequestMethod)method {
    RKLogTrace(@"cancelAllObjectRequestOperationsWithMethod : %@", RKStringFromRequestMethod(method));
    for (RKObjectRequestOperation *operation in [self enqueuedObjectRequestOperationsWithMethod:method]) {
        [operation cancel];
    }
}
