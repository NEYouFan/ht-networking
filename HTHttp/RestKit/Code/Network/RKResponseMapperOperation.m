...
@property (nonatomic, copy) void(^didFinishMappingBlock)(RKMappingResult *, id, NSError *);
@end
  ...
  responseDescriptors:(NSArray *)responseDescriptors;
{
    ...
    if (self) {
        ...
        // LWANG COMMENTS: 获取和Request对应的ResponseDescriptor和Mapping Dictionary.
        // 如果传入的ResponseDescriptor和Request不对应，那么这里会出问题并导致最终获取不到Mapping结果.
...
- (void)willFinish
{
    ...
    @synchronized(self) {
        if (self.didFinishMappingBlock) {
            if (self.error) {
                self.didFinishMappingBlock(nil, self.responseObject, self.error);
            } else {
                self.didFinishMappingBlock(self.mappingResult, self.responseObject, nil);
            }
...
- (void)main
{
    // LWANG Comments: RKResponseMapperOperation的具体执行部分也即为Mapping的核心执行部分.
    ...
    // Parse the response
    NSError *error;
    // LWANG Comments: 此处获取到待Mapping的Response数据. 该数据已经被反序列化，例如，已经转换成为了JSON格式.
    id parsedBody = [self parseResponseData:&error];
    self.responseObject = parsedBody;
    if (self.isCancelled) return [self willFinish];
    if (! parsedBody) {
    ...
    // 如果没有任何response descriptors匹配或者有匹配的ResponseDescriptor但是解析出错了, 不报错，给出警告日志信息.
    BOOL ignoreError = (error.code == RKMappingErrorNotFound && [self.responseMappingsDictionary count] == 0);
    ...
    if (!self.mappingResult) {
        ...
        if (error.code == RKMappingErrorNotFound && [self.responseMappingsDictionary count] == 0) {
            ...
            logError = [[NSError alloc] initWithDomain:RKErrorDomain code:RKMappingErrorNotFound userInfo:userInfo];
        } else {
            logError = error;
        }
        
        if (nil != logError) {
            RKLogWarning(@"Failed to get mapping result: %@", logError);
        }
    }
#endif
    
    // LWANG Comments: 如果无法取到Mapping结果，则设置Error便于正确回调结果.
    if (!self.mappingResult && !ignoreError) {
        self.error = error;
    }
...
- (RKMappingResult *)performMappingWithObject:(id)sourceObject error:(NSError **)error
{
    ...
    // LWANG Comments: 从RKObjectResponseMapperOperation中转到RKMapperOperation中执行具体的Mapping过程.
    // Note: RKMapperOperation仍然是从NSOperation中派生.
    [self.mapperOperation start];
    
    // TODO lwang: 这里没有正确的将error返回. 错误信息在self.mapperOperation.errors中而不是在self.mapperOperation.error中.
    // 导致的结果: 如果返回的Json信息和Model的Map不匹配，那么不会有任何错误信息出来，只是没有取到任何正确的数据. 不确定这一行为是否和RestKit的设计契合.
    // 当然，在解析过程中会有错误日志打印出来.
