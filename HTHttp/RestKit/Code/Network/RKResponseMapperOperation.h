...
/**
 ...
 @param block A block object to be executed when the response mapping is finished. The block has no return value and accepts three arguments: an `RKNappingResult` object that was mapped from the response or an `NSError` error indicating that the mapping has failed. Argument responseObject indicates the pared body of HTTP response. For example, it should be a dictionary if http response content is in JSON Format.
 */
- (void)setDidFinishMappingBlock:(void(^)(RKMappingResult *mappingResult, id responseObject, NSError *error))block;
