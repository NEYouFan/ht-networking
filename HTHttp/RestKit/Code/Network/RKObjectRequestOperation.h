 ...
 *  @param  待校验的operation.
 *
 *  @return 如果operation的结果有效，返回YES; 否则返回NO.
 */
@class RKObjectRequestOperation;

typedef BOOL (^HTValidResultBlock)(RKObjectRequestOperation *operation);
...
/**
 *  解析出来的response对象. 例如，如果返回的是JSON数据，那么responseObject就是一个字典.
 */
...
/**
 *  通过把success block和failure block设置为nil的方式来打破循环引用. 
 *  正常情况下不需要调用，但在上层封装了request并且request持有了operation时需要通过clearCompletionBlocks来打破循环引用.
 *  SDK使用者不需要显式调用该方法.
 */
