...
- (BOOL)matchesAbsoluteURL:(NSURL *)URL parsedArguments:(NSDictionary **)outParsedArguments {
    // pathPattern为nil时，返回YES.
    if (nil == self.pathPattern) {
        return YES;
    }
    
    // Note: 1 这里不能做直接比较. 因为URL里面可能是新加了参数, matchsPath可以正确处理. 2 加上http是避免做无谓的比较.
    return [[self.pathPattern lowercaseString] hasPrefix:@"http"] && [self matchesPath:URL.absoluteString parsedArguments:outParsedArguments];
}
...
{
    if (![self matchesURL:response.URL parsedArguments:outParsedArguments] && ![self matchesAbsoluteURL:response.URL parsedArguments:outParsedArguments]) {
        return NO;
    }
