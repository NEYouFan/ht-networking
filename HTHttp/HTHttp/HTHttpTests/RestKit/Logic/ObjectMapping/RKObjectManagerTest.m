...
@interface RKTestRequestProvider : RKRequestProvider
@end

@implementation RKTestRequestProvider
...
- (void)testInitializationWithRKRequestProviderSetsNilAcceptHeaderValue
{
...
- (void)testDefersToRKRequestProviderParameterEncodingWhenInitializedWithRKRequestProvider
{
    RKRequestProvider *requestProvider = [RKRequestProvider requestProviderWithBaseURL:[NSURL URLWithString:@"http://restkit.org"]];
...
{
    RKRequestProvider *requestProvider = [RKRequestProvider requestProviderWithBaseURL:[NSURL URLWithString:@"http://restkit.org"]];
    requestProvider.parameterEncoding = RKPropertyListParameterEncoding;
    RKObjectManager *manager = [[RKObjectManager alloc] initWithRequestProvider:requestProvider];
    expect([manager requestSerializationMIMEType]).to.equal(RKMIMETypeFormURLEncoded);
}
...
{
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:@"/object_manager/cancel" relativeToURL:self.objectManager.requestProvider.baseURL]];
...
{
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:@"/object_manager/1234/cancel" relativeToURL:self.objectManager.requestProvider.baseURL]];
    RKObjectRequestOperation *operation = [[RKObjectRequestOperation alloc] initWithRequest:request responseDescriptors:self.objectManager.responseDescriptors];
    [_objectManager enqueueObjectRequestOperation:operation];
...
{
    NSURLRequest *request1 = [NSURLRequest requestWithURL:[NSURL URLWithString:@"/object_manager/cancel" relativeToURL:self.objectManager.requestProvider.baseURL]];
...
- (void)testRKRequestProviderCanModifyRequestsBuiltByObjectManager
{
    RKTestRequestProvider *testRequestProvider = [[RKTestRequestProvider alloc] initWithBaseURL:[NSURL URLWithString:@"http://test.com"]];
    RKObjectManager *manager = [[RKObjectManager alloc] initWithRequestProvider:testRequestProvider];
    RKHuman *temporaryHuman = [RKTestFactory insertManagedObjectForEntityForName:@"Human" inManagedObjectContext:nil withProperties:nil];
