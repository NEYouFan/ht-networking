...
- (void)setUp
{
    ...
    // 为避免超时导致单元测试失败，设置异步测试默认等待时间为两秒.
    // Comment by LWANG: 单元测试因为超时失败的原因不明，所有的单元测试添加watiUntilFinished来保证请求正确.
...
{
    // Comment by LWANG: Currently there is no error reported if there is no response descriptor matched. There are only logs for such cases.
    // So this unit test case is useless now.
    // TODO: Check whether we should differ there is no response descriptor provided from provided response descriptors are incorrect.
...
- (void)testThatCacheEntryIsNotFlaggedWhenMappingFails
{
    ...
    [requestOperation waitUntilFinished];
    expect([requestOperation isFinished]).will.beTruthy();
    // Comment by LWANG: We've modified the logic about missing response descriptor.
    // In this case, there is no error reported except for some log messages. Sometimes there is no response descriptor provided and users can still use the json returned from http server.
    expect(requestOperation.error).to.beNil();
...
- (void)testThatOperationDependenciesAreRespected
{
    ...
    [requestOperation1 waitUntilFinished];
