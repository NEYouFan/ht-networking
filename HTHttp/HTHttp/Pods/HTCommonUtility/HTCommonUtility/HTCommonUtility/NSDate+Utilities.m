...
static const NSString * DateBlockKey = @"DateblockKey";
...
+ (void)netWorkDateWithCompleteBlock:(DateBlock)dateBlock
{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        NSArray *_months = @[@"",@"Jan",@"Feb",@"Mar",@"Apr",@"May",@"Jun",@"Jul",@"Aug",@"Sep",@"Oct",@"Nov",@"Dec"];
        NSHTTPURLResponse *httpResponse=(NSHTTPURLResponse *)response;
        if ([response respondsToSelector:@selector(allHeaderFields)]) {
            NSDictionary *dic=[httpResponse allHeaderFields];
            NSString *time=[dic objectForKey:@"Date"];
            NSString *timeSub=[time substringFromIndex:16];
            NSString *timeOnly=[timeSub substringToIndex:9];
            NSArray *array=[time componentsSeparatedByString:@" "];
            NSDateComponents *dateComponents = [[NSDateComponents alloc] init];
            dateComponents.month = [_months indexOfObject:array[2]];
            dateComponents.day = [array[1] integerValue];
            dateComponents.year = [array[3] integerValue];
            NSArray *arrayTest=[timeOnly componentsSeparatedByString:@":"];
            int hour=[[arrayTest objectAtIndex:0] intValue];
            int minute=[[arrayTest objectAtIndex:1] intValue];
            int second=[[arrayTest objectAtIndex:2] intValue];
            dateComponents.hour = hour;
            dateComponents.minute = minute;
            dateComponents.second = second;
            NSCalendar *greCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
            [greCalendar setTimeZone:[NSTimeZone timeZoneWithName:@"GMT"]];
            NSDate *now = [greCalendar dateFromComponents:dateComponents];
            if (dateBlock) {
                dateBlock(now);
            }
        }
